...
int main(int argc, char **argv) {
    ...
    double segment_length_spin = -1.57079633; //desired travel distance in meters; anticipate travelling multiple segments
    ...
    double dist_const_v = segment_length_1 - dist_accel - dist_decel; //if this is <0, never get to full spd
    ...
    while (ros::ok()) // do work here in infinite loop (desired for this example), but terminate if detect ROS has faulted (or ctl-C)
    {
        ...
        double dist_to_go = segment_length_1 - segment_length_done;
